...
func ValidatePlatform(p *nutanix.Platform, fldPath *field.Path) field.ErrorList {
	...
	if len(p.PrismCentral) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("prismCentral"), "must specify the name of the Prism Central"))
	}
	if len(p.PrismElement) == 0 {
		allErrs = append(allErrs, field.Required(fldPath.Child("prismElement"), "must specify the name of the Prism Element"))
	}
	if len(p.Username) == 0 {
	...
	if len(p.PrismCentral) != 0 {
		if err := validate.Host(p.PrismCentral); err != nil {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("prismCentral"), p.PrismCentral, "must be the domain name or IP address of the prismCentral"))
		}
	}
