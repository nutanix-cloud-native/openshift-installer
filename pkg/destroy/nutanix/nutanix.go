...
const (
	createdByOpenshiftInstallerDesc = "Created By OpenShift Installer"
	emptyFilter                     = ""
)
...
func (o *ClusterUninstaller) Run() (*installertypes.ClusterQuota, error) {
	...
	//Delete VMs
	err := cleanupVMs(clientV3, category, o)
	if err != nil {
		return nil, err
	}

	//Delete image
	err = cleanupImages(clientV3, category, o, imagesToDelete)
	if err != nil {
		return nil, err
	}
	//Delete category
	err = cleanupCategory(clientV3, category, o)
	if err != nil {
		return nil, err
	}
	return nil, nil
		...
		if strings.HasPrefix(vmName, o.InfraID) {
			if hasCategoryAssigned(v.Metadata, ec, o.Logger) {
				if v.Spec.Description != nil {
					if *v.Spec.Description == createdByOpenshiftInstallerDesc {
			...
			if imageName == expectedImageName {
				if hasCategoryAssigned(i.Metadata, ec, o.Logger) {
					if i.Spec.Description != nil {
						if *i.Spec.Description == createdByOpenshiftInstallerDesc {
							found = true
							...
							if err != nil {
								o.Logger.Warnf("error while deleting images: %s", err)
							}
							break
						}
					}
