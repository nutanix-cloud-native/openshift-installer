...
func Platform() (*nutanix.Platform, error) {
	...
	pe, peUUID, err := getPrismElement(ctx, v3Client)
	if err != nil {
		return nil, err
	}
	subnet, err := getSubnet(ctx, v3Client, peUUID)
	if err != nil {
		return nil, err
	}
	...
	platform := &nutanix.Platform{
		...
		Insecure:     nutanixClient.Insecure,
	...
	if err := survey.Ask([]*survey.Question{
		{
			Prompt: &survey.Input{
				Message: "Port",
				Help:    "The port used to to login to the Prism Central.",
				Default: "9440",
			},
			Validate: survey.Required,
		},
	...
	if err := survey.Ask([]*survey.Question{
		{
			Prompt: &survey.Confirm{
				...
				Help:    "Connect to Prism Central with certificate checking",
			},
			Validate: survey.Required,
		},
	}, &secure); err != nil {
...
func getPrismElement(ctx context.Context, client *nutanixClientV3.Client) (string, string, error) {
	...
	return selectedPe, peUUIDs[selectedPe], nil
...
func getSubnet(ctx context.Context, client *nutanixClientV3.Client, peUUID string) (string, error) {
	...
	v3 := client.V3
	emptyFilter := ""
	subnetsAll, err := v3.ListAllSubnet(emptyFilter)
	if err != nil {
